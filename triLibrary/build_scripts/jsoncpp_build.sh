...
cd jsoncpp
echo build jsoncpp dir: `pwd`
if [ -d $BUILD_DIR_NAME ]; then
    echo "remove the old jsoncpp build dir......."
 	rm -rf $BUILD_DIR_NAME
fi
mkdir $BUILD_DIR_NAME
cd $BUILD_DIR_NAME
# 编译选项Debug/release
JSONCPP_BUILD_CONFIGURE=$BUILD_CONFIGURE
# 因为3方库通常都不会一直更改,所以只要已经编译完成,则直接退出
if [ -e "$INSTALL_DIR/include/json/json.h" ]; then
...
$CMAKE_TOOL -G "Unix Makefiles" -DCMAKE_BUILD_TYPE="$JSONCPP_BUILD_CONFIGURE" \
